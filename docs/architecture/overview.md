...
```text
  ...
  | Next.js / HeroUI     |                            | API / RBAC / worker       |
  +----------------------+                            +------------+--------------+
                                                                 |       |
                                                    +------------+       +------------------+
                                                    v                                   v
                                         +----------------------+          +----------------------+
                                         | PostgreSQL           |          | ACME / DNS / Aliyun   |
                                         | config / secrets /   |          | SSL / ALB providers   |
                                         | jobs / audit records |          +----------------------+
                                         +----------------------+

  Future managed network
  +----------------------+       agent.v1 protocol    +---------------------------+
  | Agent                | ------------------------> | Control Plane             |
  | outbound connection  |                            | no direct DB access       |
  +----------------------+                            +---------------------------+
```
...
| 阿里云 | 官方 Go SDK v2 |
| 迁移 | `golang-migrate`，迁移嵌入 Control Plane 镜像并在启动时自动执行 |
| 日志 | 标准库 `log/slog`，结构化 JSON 输出 |
| 指标 | Prometheus `/metrics` |
| Console | Next.js + React + HeroUI 3.0 |
...
certflow/
  apps/
    control-plane/         # API、调度器、worker、PostgreSQL 迁移
    console/               # Next.js App Router 管理控制台
    agent/                 # 未来内网执行面，独立 Go Module
  contracts/
    control-plane/         # Console API 契约
    agent/v1/              # Agent 注册、任务与结果协议
  docs/
  deploy/
    compose/               # 本地与生产 Compose
    proxy/                 # 反向代理模板
  tools/                   # 诊断与受控运维脚本
```

Control Plane 位于 `apps/control-plane/`，Next.js Console 位于 `apps/console/`；二者独立构建和部署。未来 Agent 作为独立模块部署在受管网络中，只通过版本化契约与 Control Plane 通信，不能访问数据库或导入 Control Plane 的 `internal` 包。Control Plane 不将 ACME、DNS 或阿里云 SDK 调用直接放入 HTTP handler。handler 只负责鉴权、参数校验、创建用例请求和返回任务 ID。

### 4.1 Console

Console 使用 Next.js、React 与 HeroUI 3.0，是受 RBAC 保护的操作界面；不承载证书签发、DNS 验证、续期、部署或密钥加密等业务逻辑，这些操作全部由 Control Plane 的 API 和 worker 执行。
...
Console 采用 Next.js App Router。优先使用 React Server Components 获取首次页面数据，仅将筛选、表单、确认弹窗、轮询中的执行详情等需要浏览器交互的区域实现为 Client Components。HeroUI 3.0 提供表格、表单、Select、Modal、Drawer、Tabs、Toast、Badge 和 Skeleton 等基础 UI；共享组件按 `components/layout`、`components/providers`、`components/ui`、`components/charts` 与 `components/marketing` 分层，领域代码逐步收敛到 `apps/console/features/`。
...
- 每个 SAN 可继承默认 DNS 账户或选择其 Zone 对应的账户；Console 仅展示 Control Plane 已验证可管理的 Zone，不自行推断 DNS 归属。
...
- Console 使用 TanStack Query 管理客户端资源缓存，交互突变后精确失效关联查询；SSE 用于接收证书、自动化和通知的实时事件，REST API 与数据库仍是状态真相。

Console 与 Control Plane 通过版本化 REST API 通信：浏览器不直接请求 ACME、阿里云 DNS、SSL 管理或 ALB API。推荐由 Next.js BFF route handler 代理同源请求，并使用 `httpOnly`、`secure`、`sameSite=lax` session cookie；Control Plane 根据 session 或 Bearer token 进行 RBAC 判定。若 Console 与 Control Plane 以不同域名部署，必须配置精确 CORS allowlist 和 CSRF 防护，不能使用通配符来源。

Control Plane API 应维护 OpenAPI 规范，Console 从规范生成 TypeScript 类型和 API client，避免手写 DTO 漂移。错误响应包含稳定 `code`、用户可读 `message` 与 `request_id`；Console 将 `request_id` 展示在可复制的错误详情中，便于排障。
...
1. 单仓库骨架：Go `apps/control-plane/`、Next.js `apps/console/`、Docker Compose、配置加载、PostgreSQL 迁移、本地邮箱账户、RBAC 和 envelope encryption。
