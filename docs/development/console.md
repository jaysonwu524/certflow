# CertFlow Console 工程规范

本文档是管理台界面与交互的当前规范，不记录阶段性完成进度。页面实现、组件重构和待办事项通过 Issue、PR 和 CHANGELOG 跟踪。
...
非目标：本轮不改变 Control Plane 的签发、续期、部署领域规则；Console 只补充完成交互所必需的查询与修改 API。
...
apps/console/
  ...
  features/
    ...
    credentials/ credential-modal.tsx acme-account-modal.tsx dns-account-modal.tsx
    auth/ login-form.tsx register-form.tsx
    settings/ smtp-settings.tsx profile-security.tsx
  lib/
...
数据访问采用 TanStack Query：Query Key 统一定义，Mutation 成功后精确失效相关资源，失败映射成稳定的 `ApiError`。Control Plane 应维护 OpenAPI 描述并生成 TypeScript Client，避免当前页面重复定义 API 类型。
...
删除 API 必须返回引用冲突的结构化原因；不能删除时，Console 在 ConfirmDialog 中展示引用资源。所有 API 错误统一返回 `code`、`message`、`fieldErrors`、`requestId`，Console 按错误类型区分认证、校验、冲突和网络故障。
