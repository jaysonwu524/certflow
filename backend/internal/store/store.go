...
func (s *Store) Dashboard(ctx context.Context) (domain.Dashboard, error) {
	...
	err := s.pool.QueryRow(ctx, `
		SELECT
			...
			(SELECT count(*) FROM certificates c JOIN certificate_versions v ON v.id = c.current_certificate_version_id WHERE v.not_after <= now() + interval '30 days' AND v.not_after > now() AND c.deleted_at IS NULL),
			(SELECT count(*) FROM jobs WHERE status = 'running'),
			(SELECT count(*) FROM workflow_executions WHERE status = 'failed' AND started_at >= now() - interval '7 days')
	`).Scan(&dashboard.ActiveCertificates, &dashboard.ExpiringSoon, &dashboard.RunningJobs, &dashboard.FailedExecutions)
	return dashboard, err
}

func (s *Store) ListCertificates(ctx context.Context) ([]domain.CertificateSummary, error) {
	rows, err := s.pool.Query(ctx, `
		SELECT c.id, c.name, c.status, c.key_algorithm, c.validation_mode, c.renew_enabled, v.not_after, COALESCE(v.sha256_fingerprint, ''), c.last_issued_at, COALESCE(c.last_error, ''), c.created_at,
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&certificate.ID, &certificate.Name, &certificate.Status, &certificate.KeyAlgorithm, &certificate.ValidationMode, &certificate.RenewEnabled,
...
func (s *Store) CreateCertificate(ctx context.Context, input domain.CreateCertificateInput) (string, error) {
	...
	if _, err := tx.Exec(ctx, `
		INSERT INTO certificates (id, name, acme_account_id, default_dns_account_id, key_algorithm, challenge_type, renew_enabled, renew_before_days, validation_mode, status, created_at, updated_at)
		VALUES ($1, $2, NULLIF($3, '')::uuid, NULLIF($4, '')::uuid, $5, 'dns_01', $6, $7, $8, 'pending', $9, $9)
	`, certificateID, input.Name, input.AcmeAccountID, input.DefaultDNSAccountID, input.KeyAlgorithm, input.RenewEnabled, input.RenewBeforeDays, input.ValidationMode, now); err != nil {
		return "", err
	}
...
func (s *Store) GetManualChallenge(ctx context.Context, certificateID string) (ManualChallenge, error) {
	...
	err := s.pool.QueryRow(ctx, `
		...
		WHERE certificate_id = $1 AND status IN ('waiting_user', 'approved')
		ORDER BY created_at DESC LIMIT 1
	`, certificateID).Scan(&challenge.ID, &challenge.JobID, &challenge.CertificateID, &challenge.OrderURL, &authz, &items, &challenge.Status)
	if err != nil {
...
func (s *Store) ApproveManualChallenge(ctx context.Context, certificateID string) error {
	...
	defer tx.Rollback(ctx)
	var jobID string
	if err := tx.QueryRow(ctx, `SELECT job_id FROM manual_validation_challenges WHERE certificate_id = $1 AND status = 'waiting_user' ORDER BY created_at DESC LIMIT 1 FOR UPDATE`, certificateID).Scan(&jobID); err != nil {
		return err
	}
...
func (s *Store) CreateCloudCredential(ctx context.Context, credentialID, versionID string, input domain.CreateCloudCredentialInput, hint string, ciphertext []byte) error {
	...
	if _, err := tx.Exec(ctx, `
		INSERT INTO cloud_credentials (id, name, provider, credential_hint, status, current_version_id, created_at, updated_at)
		VALUES ($1, $2, 'aliyun', $3, 'active', NULL, $4, $4)
	`, credentialID, input.Name, hint, now); err != nil {
		return err
	}
	if _, err := tx.Exec(ctx, `
...
func (s *Store) LoadCloudCredentialMaterial(ctx context.Context, credentialID string) (CloudCredentialMaterial, error) {
	...
	`, credentialID).Scan(&material.VersionID, &material.CredentialsCiphertext, &material.Status)
	return material, err
}
...
func (s *Store) CreateACMEAccount(ctx context.Context, accountID string, input domain.CreateACMEAccountInput, ciphertext []byte) error {
	_, err := s.pool.Exec(ctx, `
		INSERT INTO acme_accounts (id, name, directory_url, email, private_key_ciphertext, private_key_algorithm, status)
		VALUES ($1, $2, $3, $4, $5, $6, 'active')
	`, accountID, input.Name, input.DirectoryURL, input.Email, ciphertext, input.PrivateKeyAlgorithm)
	return err
}
...
func (s *Store) CreateDNSAccount(ctx context.Context, accountID string, input domain.CreateDNSAccountInput) error {
	...
	if err := s.pool.QueryRow(ctx, `SELECT EXISTS (SELECT 1 FROM cloud_credentials WHERE id = $1 AND status = 'active' AND deleted_at IS NULL)`, input.CloudCredentialID).Scan(&validCredential); err != nil {
		return err
	}
	if !validCredential {
	...
	_, err := s.pool.Exec(ctx, `
		INSERT INTO dns_accounts (id, name, provider, cloud_credential_id, allowed_zones, status)
		VALUES ($1, $2, 'aliyun', $3, $4, 'active')
	`, accountID, input.Name, input.CloudCredentialID, input.AllowedZones)
	return err
}
...
func (s *Store) ListDeploymentTargets(ctx context.Context) ([]domain.DeploymentTargetSummary, error) {
	rows, err := s.pool.Query(ctx, `
		...
		FROM deployment_targets WHERE deleted_at IS NULL ORDER BY created_at DESC
	`)
	if err != nil {
		return nil, err
	}
...
func (s *Store) CreateDeploymentTarget(ctx context.Context, targetID string, input domain.CreateDeploymentTargetInput, listenerProtocol string) error {
	...
	result, err := s.pool.Exec(ctx, `
		INSERT INTO deployment_targets (id, name, type, cloud_credential_id, config, status)
		SELECT $1, $2, 'aliyun_alb', $3, $4::jsonb, 'active'
		WHERE EXISTS (SELECT 1 FROM cloud_credentials WHERE id = $3 AND status = 'active' AND deleted_at IS NULL)
	`, targetID, input.Name, input.CloudCredentialID, config)
	if err != nil {
		return err
	}
...
func (s *Store) CreateCertificateDeployment(ctx context.Context, deploymentID string, input domain.CreateCertificateDeploymentInput) error {
	result, err := s.pool.Exec(ctx, `
		INSERT INTO certificate_deployments (id, certificate_id, deployment_target_id, auto_deploy, enabled)
		SELECT $1, $2, $3, $4, true
		WHERE EXISTS (SELECT 1 FROM certificates WHERE id = $2 AND deleted_at IS NULL)
		  AND EXISTS (SELECT 1 FROM deployment_targets WHERE id = $3 AND status = 'active' AND deleted_at IS NULL)
	`, deploymentID, input.CertificateID, input.TargetID, input.AutoDeploy)
	if err != nil {
		return err
	}
	if result.RowsAffected() == 0 {
...
func (s *Store) QueueDeployment(ctx context.Context, deploymentID string) error {
	...
	err := s.pool.QueryRow(ctx, `
		...
		WHERE d.id = $1 AND d.enabled AND c.current_certificate_version_id IS NOT NULL
	`, deploymentID).Scan(&certificateID, &targetID, &versionID)
	if err != nil {
		return err
	}
...
func (s *Store) LoadDeploymentConfiguration(ctx context.Context, certificateID, targetID string) (DeploymentConfiguration, error) {
	var config DeploymentConfiguration
	err := s.pool.QueryRow(ctx, `
		SELECT c.id, t.id, t.name, t.config->>'region_id', t.config->>'load_balancer_id', t.config->>'listener_id',
		...
		JOIN deployment_targets t ON t.id = $2 AND t.status = 'active' AND t.deleted_at IS NULL
	...
	`, certificateID, targetID).Scan(&config.CertificateID, &config.TargetID, &config.TargetName, &config.RegionID, &config.LoadBalancerID, &config.ListenerID,
...
func (s *Store) LoadIssueConfiguration(ctx context.Context, certificateID string) (IssueConfiguration, error) {
	...
	rows, err := s.pool.Query(ctx, `SELECT domain FROM certificate_domains WHERE certificate_id = $1 ORDER BY position`, certificateID)
	if err != nil {
		return IssueConfiguration{}, err
	}
	defer rows.Close()
...
func (s *Store) ClaimJob(ctx context.Context, workerID string, leaseDuration time.Duration) (*ClaimedJob, error) {
	...
	err = tx.QueryRow(ctx, `
		...
		RETURNING j.id, j.kind, j.payload, j.attempt, j.max_attempts,
			COALESCE(j.payload->>'certificate_id', ''), COALESCE(j.payload->>'deployment_target_id', '')
	`, workerID, intervalLiteral(leaseDuration)).Scan(&job.ID, &job.Kind, &job.Payload, &job.Attempt, &job.MaxAttempts, &job.CertificateID, &job.DeploymentTargetID)
	if err != nil {
	...
	if _, err := tx.Exec(ctx, `
		INSERT INTO workflow_executions (id, job_id, kind, certificate_id, deployment_target_id, trigger_type, status, attempt, max_attempts, started_at)
		VALUES ($1, $2, $3, NULLIF($4, '')::uuid, NULLIF($5, '')::uuid, $6, 'running', $7, $8, now())
	`, job.ExecutionID, job.ID, job.Kind, job.CertificateID, job.DeploymentTargetID, trigger, job.Attempt, job.MaxAttempts); err != nil {
		return nil, err
	}
